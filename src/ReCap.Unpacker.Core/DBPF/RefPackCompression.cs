...
{
    public static class RefPackCompression
    {
        static readonly ushort[] CrcTab = new ushort[]
        {
        ...
        public static int GetDecompressedSize(ReCap.Unpacker.Core.FileStructures.IStreamReader s)
        {
            byte cType = s.ReadByte();
            s.Skip(1);
            if (cType == 0x10 || cType == 0x50)
            {
                int v1 = s.ReadByte() & 0xFF;
                int v2 = s.ReadByte() & 0xFF;
                int v3 = s.ReadByte() & 0xFF;
                int decomp = (v1 << 16) | (v2 << 8) | v3;
                s.Skip(-5);
                return decomp;
            }
            else
        ...
        public static void DecompressFast(byte[] input, byte[] output)
        {
            int pin = 0;
            ...
            while (size < decompSize)
            {
                ...
                if (numPlain > 0)
                {
                    Buffer.BlockCopy(input, pin, output, size, numPlain);
                    pin  += numPlain;
                    size += numPlain;
                }
                ...
                if (numCopy > 0)
                {
                    ...
                    {
                        Buffer.BlockCopy(output, size - copyOff, output, size, numCopy);
                        size += numCopy;
                    }
                }
            }
